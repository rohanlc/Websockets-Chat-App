...
function process($user,$msg){
  ...
  say("< ".$action);
  switch($action){
    case "hello" : send($user->socket,var_dump($user->socket));                       break;
    case "hi"    : send($user->socket,serialize($us));                         break;
    case "name"  : send($user->socket,var_dump($users)); break;
    case "age"   : send($user->socket,var_dump($user));       break; 
    case "date"  : send($user->socket,"today is ".date("Y.m.d"));           break;
    case "time"  : send($user->socket,"server time is ".date("H:i:s"));     break;
    case "thanks": send($user->socket,serialize($sockets));                    break;
